...
const ProductVariantType = ({isMobile, variantSlug, updateTerminalVariant }) => {
  const { variants } = useContext(ProductContext);
  const defaultSku = getVariantSkuBySlug(variants, variantSlug);
  const [selectedSku, dispatch] = useReducer(productTypeReducer, { 'Product Type': defaultSku} );
  ...
  return (
    <>
      <VariantType isMobile={isMobile} variantType="Product Type" options={variantOptions} variantValue={selectedSku['Product Type']} dispatch={dispatch}/>
      <br/>
    </>
  );
};
